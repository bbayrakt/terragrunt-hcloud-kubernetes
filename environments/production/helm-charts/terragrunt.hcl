...
terraform {
  ...
  before_hook "require_cluster_kubeconfig" {
    ...
    execute = [
      "bash",
      "-lc",
      "if [ ! -f '${local.fallback_kubeconfig_path}' ]; then echo 'Helm charts module requires an existing Kubernetes cluster kubeconfig at ${local.fallback_kubeconfig_path}. Apply environments/production/kubernetes-cluster first.' >&2; exit 1; fi"
    ]
  }
}
