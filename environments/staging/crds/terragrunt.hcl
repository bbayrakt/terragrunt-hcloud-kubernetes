...
terraform {
  ...
  before_hook "require_cluster_kubeconfig" {
    ...
    execute = [
      "bash",
      "-lc",
      "if [ ! -f '${local.fallback_kubeconfig_path}' ]; then echo 'CRDs module requires an existing Kubernetes cluster kubeconfig at ${local.fallback_kubeconfig_path}. Apply environments/staging/kubernetes-cluster first.' >&2; exit 1; fi"
    ]
  }
